...
# data_path = '/external/rprshnas01/netdata_kcni/jglab/MemberSpaces/Data/Shrey/WhoBPyT/200_subjects_WhoBPyT_task_run_pkls_fake_time'
...
filename = data_path + '/Subj_{0}_fittingresults_{1}_task_stim_exp.pkl'.format(sub_id, task_choice)
with open(filename, 'rb') as f:
    ss_og_data = pickle.load(f)

filename1 = data_path + '/Subj_{0}_fittingresults_{1}_task_stim_exp.pkl'.format(sub_id, task_choice)
...
# As per conversation with Zheng, doing F.test(20), and then F.test_realtime(), potentially yeilds better sims for the realtime timeseries but for lesioned SC. NOT original whobpyt sims for rest/task fMRI. 
...
# ## In this script, we use ntwx_iso_data.test(20) instead of ntwx_iso_data.test_realtime(720,0.04,0.04,60)
...
# # filename = data_path + '/Subj_{0}_fittingresults_stim_exp_real_time.pkl'.format(sub_id)
# # Subj_121921_fittingresults_stim_exp_real_time.pkl
...
# output_path = '/external/rprshnas01/netdata_kcni/jglab/MemberSpaces/Data/Shrey/WhoBPyT/Ntwx_Lesion_REST_WhoBPyT_real_time_pkls_and_conn_mats'
